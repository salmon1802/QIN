...
class QIN_variety_v9(BaseModel):
    def __init__(self,
        ...
        self.factor = factor
        self.pseudo_label_epochs = pseudo_label_epochs
        self.embedding_dim = embedding_dim
        self.item_info_dim = 0
        for feat, spec in self.feature_map.features.items():
            if spec.get("source") == "item":
                self.item_info_dim += spec.get("embedding_dim", embedding_dim)
        self.item_info_dim += embedding_dim * 2 # embedding_dim * 4 + 128
        ...
        input_dim = feature_map.sum_emb_out_dim() + embedding_dim * 2 # embedding_dim * 7 + 128
        input_dim = input_dim + self.item_info_dim
